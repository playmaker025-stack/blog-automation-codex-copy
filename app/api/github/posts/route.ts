...
async function loadIndex(): Promise<{ data: PostingIndex; sha: string | null }> {
  // 한 번만 읽는다. 예전에는 fileExists로 한 번, readJsonFile로 또 한 번
  // 같은 383KB를 내려받았다. 저장할 때마다 그 왕복이 두 배였다.
  const file = await readFileOrNull(Paths.postingListIndex());
  if (!file) {
    return { data: { ...EMPTY_INDEX, lastUpdated: new Date().toISOString() }, sha: null };
  }
  return { data: JSON.parse(file.content) as PostingIndex, sha: file.sha };
}
...
async function resolveTargetKeywords(
  ...
  try {
    const file = await readFileOrNull(Paths.topicsIndex());
    if (!file) return normalizeTargetKeywords(fallback);
    const data = JSON.parse(file.content) as TopicIndex;
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (typeof content === "string") {
      ...
      const contentSha = (await readFileOrNull(contentPath))?.sha ?? null;
      await writeFile(
        contentPath,
        content,
    ...
    return NextResponse.json({
      updated: true,
      // 고친 글을 돌려준다. 화면이 목록 전체를 다시 받지 않고 이 한 건만 갈아끼우면 된다.
      post: updatedPost,
