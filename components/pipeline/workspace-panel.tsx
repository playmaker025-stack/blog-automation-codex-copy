...
function summarizeKeywordRow(keywordReport: KeywordUsageReport): string {
  return keywordReport.items
    .filter((item) => item.role !== "anchor" && item.role !== "forbidden")
    .slice(0, 4)
                  ...
                  {draftColumns.map((column, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {versionReport && (() => {
                          const contractItems = versionReport.keywordReport.items.filter(
                            (item) => item.role !== "anchor" && item.role !== "forbidden"
                          );
                          const tokenItems = versionReport.keywordReport.tokenItems ?? [];
                          const showTokenFallback = contractItems.length <= 1 && tokenItems.length > 0;
                          return (
                            <div className="space-y-3 border-t border-zinc-100 bg-white px-4 py-3">
                              ...
                              <div className="space-y-2">
                                ...
                                {showTokenFallback && tokenItems.slice(0, 6).map((tok) => (
                                  <div
                                    key={`${column.label}-tok-${tok.token}`}
                                    className="rounded-md border border-zinc-100 bg-zinc-50 px-3 py-2"
                                  >
                                    <div className="flex items-center justify-between gap-3">
                                      <p className="text-xs font-semibold text-zinc-800">{tok.token}</p>
                                      <p className="text-[11px] text-zinc-500">본문 {tok.count}회</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
