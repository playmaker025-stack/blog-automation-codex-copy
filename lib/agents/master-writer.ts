...
- 첫 2문단은 실제 손님 질문이나 상황에서 시작하되, 그 질문에 대한 답을 미루지 말고 바로 준다
- '한 번에 정리', '실패 없는 선택', '체크포인트', '핵심 포인트' 같은 AI형 표현과 소제목 금지

## AI 인용 필수 요건 (네이버 AI탭/AI 브리핑 노출 목표)
이 블로그의 목표는 네이버 AI탭과 AI 브리핑에 출처로 인용되는 것이다.
아래 네 가지는 주제와 무관하게 모든 글에 반드시 들어간다.

1. 첫 3문단 안에, 앞뒤 문맥 없이 읽어도 뜻이 통하는 독립 문장 3개
   - 검색 질문에 대한 직접 답변 1문장
   - 비교 또는 판단 기준 1문장
   - 예외 조건이나 주의점 1문장
2. 그 문장들에 숫자·가격·기간·날짜 중 최소 하나의 구체값 (확인 못 한 값은 지어내지 말고 다른 구체값으로 대체)
3. 본문 안에 목록, 단계, 표 중 최소 하나 (AI가 파싱할 수 있어야 한다)
4. 글 하단에 FAQ 2~3개. 실제 검색 질문 형태로 쓰고, 각 질문에 2~4문장으로 답한다

동시에, 인용만 당하고 클릭은 안 되는 글이 되지 않도록 실제 가격·재고·시연 결과·상담 기준은
도입 요약이 아니라 본문에만 남긴다.

## 출력 형식
전략에 따른 마크다운 본문 전체를 출력한다. 설명·메타 정보 없이 본문만 출력한다.
FAQ는 "## 자주 묻는 질문" 소제목 아래에 넣는다.
...
export function buildOpenAIWriterSystemPrompt(): string {
  return [
    ...
    "2026 Naver — AEO (AI Briefing Citation): The first two paragraphs must contain 1-2 complete, quotable sentences that directly answer the reader's core search question. Include at least one concrete fact per paragraph: a number, price, duration, or date. Naver AI Briefing extracts and cites these sentences, so they must stand alone as a clear answer.",
    "2026 Naver — Human Fingerprint (D.I.A.+ AI-content detection avoidance): Every section must include 1-2 first-person experience sentences. Required phrase patterns: '직접 써봤더니', '가격이 XX,000원이었는데', 'N일 쓰고 느낀 건', '처음엔 저도 몰랐는데', '실제로 해보니'. Without these anchors, Naver D.I.A.+ classifies the draft as AI-generated and suppresses distribution.",
    "2026 Naver — Dwell time structure: (1) Open the body with a 3-4 line informal content roadmap as a scroll anchor so readers know where to look. (2) Insert one interest-transfer sentence mid-article: '사실 여기서 많이들 실수하는데요', '처음엔 몰랐던 건데', '이게 핵심인데요'. (3) Close with a natural next-step direction tied to the bridge keyword to encourage return visits and improve C-Rank behavioral signals.",
    "2026 Naver — Structural variety (AI pattern detection): Vary sentence length, paragraph length, and section format naturally across sections. Identical structural patterns across consecutive sections are flagged by AI content detectors.",
  ].join("\n");
}
...
export function buildOpenAIWriterUserPrompt(params: {
  ...
  return [
    ...
    (() => {
      const articleType = strategy.keywordContract?.articleType ?? "";
      const isInformational = ["general_info", "howto", "problem_solution"].includes(articleType);
      return isInformational
        ? "- AEO (2026): The first 2 paragraphs must directly answer the search question in 1-2 complete, quotable sentences with at least one concrete fact (number, date, price, duration). Naver AI Briefing extracts these for citations — write them to stand alone."
        : "- Commercial intent (2026): Lead with a personal experience summary that includes actual price, usage duration, or before/after result as the first concrete anchor sentence.";
    })(),
    "- Human fingerprint (mandatory, every section): Insert 1-2 first-person experience sentences per section ('직접 써봤더니', 'N일 써보고', '가격이 X원이었는데', '처음엔 몰랐는데'). Pure-information sections with no experience anchor are classified as AI-generated by Naver D.I.A.+.",
    "- Include 1-2 counterintuitive or surprising findings across the article: things that differed from expectations, mistakes made, adjustments needed. These cannot be fabricated by a generic AI and boost human-signal credibility.",
    "- Open the body with a 3-4 line informal roadmap of what this article covers and why it matters (scroll anchor for dwell time).",
    "- Insert one mid-article interest hook: '사실 이게 핵심인데요', '여기서 많이들 실수하더라고요', '처음엔 저도 헷갈렸는데'. Place it at a natural inflection point, not forced.",
    "- Close with a next-step section tied to the bridge keyword direction that naturally encourages return visits (C-Rank behavioral signal optimization).",
