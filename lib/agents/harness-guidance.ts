...
평가 우선순위
- 검색의도 완결: 독자가 지금 궁금한 핵심 답을 이 글 안에서 충분히 해결
- 실제 손님 질문/상황: 첫 문단부터 현실적인 질문과 상담 흐름으로 연결
- 자연스러운 네이버 모바일 문체: 짧고 읽기 쉬운 문단, 과한 템플릿 소제목 지양
- 상담 흐름: 선택 기준, 실패 이유, 비교 포인트, 방문 전 확인 기준을 실제 맥락으로 정리
- 그 다음 SEO: 키워드는 유용한 문장 안에 자연스럽게 배치
...
- 일반 글이라면 핵심 답변을 "다음 글에서 자세히"로 미루지 말고 이 글 안에서 완결합니다.
- "한 번에 정리", "실패 없는 선택", "체크포인트", "핵심 포인트" 같은 AI형 표현과 소제목은 피합니다.`;
}
